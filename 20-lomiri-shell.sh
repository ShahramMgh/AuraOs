...
apt-get install -y --no-install-recommends \
  lomiri lomiri-touch-session lomiri-greeter \
  mir-platform-graphics-gbm-kms lightdm

# Make Lomiri (not GNOME/unity) the default session
if [ -f /etc/lightdm/lightdm.conf ]; then
  sed -i 's/^#\?user-session=.*/user-session=lomiri-touch/' /etc/lightdm/lightdm.conf
fi
